...
rain.cleaner <- function(x) {
  ...
  # if the number of days accumulated precip is more than one, delete that observation
  mult.days <- with(x, which(days.accum != 1 & !(is.na(days.accum))))
  x$rain[mult.days] <- NA
  x$days.accum[mult.days] <- NA
  # if the number of days accumulated precip is not recorded, delete that observation
  x$rain[which(is.na(x["days.accum"]))] <- NA
  return(x)
  }
...
# Minimum temperature
cts.min <- read.csv("climate/canopy_crane/")
# Maximum temperature
cts.max <- read.csv("climate/canopy_crane/")
# Rainfall/precipitation
cts.r.raw <- 
